    ...
    try:
        ...
        with pdfplumber.open(pdf_file) as pdf:
            for page in pdf.pages:
                ...
                if page_text and len(page_text.strip()) > 10:  # Check if meaningful text extracted
                    text += page_text + "\n"
                ...
                for table in tables:
                    for row in table:
                        ...
                        if row_text.strip():
                            text += "\n" + row_text
        if len(text.strip()) < 50:  # If little/no text extracted, try OCR
...
def preprocess_text(resume_text):
    # Normalize table separators (e.g., "|", multiple spaces, tabs)
    resume_text = re.sub(r'\|\s*', ' | ', resume_text)  # Standardize pipe separators
    resume_text = re.sub(r'\t+', ' ', resume_text)  # Replace tabs with spaces
    resume_text = re.sub(r'\s{2,}', ' ', resume_text)  # Collapse multiple spaces
    # Split table-like rows into lines
    resume_text = re.sub(r'\|', '\n', resume_text)
    return resume_text.strip()

def parse_resume(resume_text):
    print("Parsing resume text")
    # Preprocess text to handle tables and normalize spacing
            ...
            [
                {
                    ...
                    "parts": [
                        {
                            "text": (
                                "You are a resume parsing expert. Extract structured information from the resume text, including content from tables or unconventional layouts.\n"
                                "Extract the following information from this resume, formatted as JSON:\n"
...
def extract_personal_details(resume_text):
    ...
    if not personal_details["name"]:
        # Fallback: Look for capitalized words in first 10 lines
...
def find_section(resume_text, section_titles):
    ...
    for title in section_titles.split('|'):
        # Match section headers with various formats like "EXPERIENCE", "Experience:", "WORK EXPERIENCE"
    ...
    # Return the section that best matches the requested section titles
...
def extract_education_regex(resume_text):
    ...
    # Extract education entries based on paragraphs
    paragraphs = re.split(r'\n\s*\n', education_section)
    for paragraph in paragraphs:
        paragraph = paragraph.strip()
        if len(paragraph) > 10:  # Skip very short paragraphs
            # Check if paragraph contains education-related keywords
    ...
    # If we didn't find education entries through paragraphs, try with bullet points
    if not education:
...
def extract_skills_regex(resume_text):
    ...
    # Handle table-like skill entries (e.g., "Python | 3 years | Advanced")
    ...
    # Existing bullet point and comma-separated logic
    ...
    # Try extracting any technical-looking terms if no skills found
    if not skills:
...
def add_skill_to_list(skill_text, skills_list, processed_skills, category=None):
    """Helper function to process a skill and add it to the list if valid."""
    skill_text = skill_text.strip()
    
    # Skip if it looks like a sentence rather than a skill
    if not skill_text or len(skill_text) > 50 or re.search(r'\b(I|we|he|she|they|it)\b', skill_text, re.IGNORECASE):
        return
    
    # Process the skill text to extract skill name, proficiency and duration
    ...
    if category and not re.search(fr'\b{re.escape(category)}\b', skill_obj["skill"], re.IGNORECASE):
        # Try to detect if this is a category-specific entry like "Python (Programming Languages)"
        if not any(c in skill_obj["skill"].lower() for c in ["programming", "language", "framework", "database"]):
            # Keep track of the category in a new field if needed
            skill_obj["category"] = category
    
    # Use a normalized version of the skill name as key for deduplication
    skill_key = skill_obj["skill"].lower()
    
    # Only add if we haven't processed this skill already
...
def process_skill_text(skill_text):
    ...
    if duration_match:
        ...
        # If we haven't already extracted proficiency (which would have removed duration from skill name)
        if skill_obj["skill"] == skill_text.strip():
...
def extract_experience_regex(resume_text):
    ...
    # Try to identify job entries by looking for date patterns
    date_patterns = [
    ...
    for pattern in date_patterns:
        ...
        if date_matches:
            for i, match in enumerate(date_matches):
                ...
                # Look for the beginning of this entry (go back to find the job title)
                ...
                # Determine the end of this entry
                ...
                if job_entry and len(job_entry) > 30:  # Skip very short entries
                    job_entries.append(job_entry)
    
    # If we couldn't extract by dates, try to extract by paragraphs
    if not job_entries:
        paragraphs = re.split(r'\n\s*\n', experience_section)
        for paragraph in paragraphs:
            paragraph = paragraph.strip()
...
def analyze_with_gemini(resume_text, job_description, job_role):
            ...
            [
                {
                    ...
                    "parts": [
                        {
                            "text": (
                                "You are an ATS expert that analyzes resumes against job descriptions. Provide detailed analysis including match percentage, matching keywords, missing keywords, and suggestions for improvement.\n"
                                f"Job Role: {job_role}\n\n"
                                f"Job Description: {job_description}\n\n"
                                f"Resume: {resume_text}\n\n"
                                "Please analyze this resume against the job description and provide in JSON format: "
...
async def analyze_resume(
    ...
    try:
        ...
        if job_description.strip():
            ...
            analysis_result = analyze_with_gemini(resume_text, job_description, job_role)
            response["analysis"] = analysis_result
        else:
            print("No job description provided, adding placeholder message")
            # Add a meaningful placeholder message instead of omitting the key
